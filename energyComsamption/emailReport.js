...
// Force postgres for email report
...
// ── Config ────────────────────────────────────────────────
const MAIN_BREAKERS = [
  { id: 1,  name: "B0 — Main",   group: "B0",   color: "#CC0010" },
  { id: 22, name: "Roof — Main", group: "Roof",  color: "#f0a000" },
  { id: 27, name: "PB — Main",   group: "PB",    color: "#2255bb" },
  { id: 30, name: "PV Panels",   group: "PV",    color: "#1a7f37" },
];

const PB1_BREAKERS = [
  { id: 28, name: "PB1 — Main" },
  { id: 29, name: "PB1 — AC Charges" },
];
...
async function getBreaker(id, from, to) {
  try {
    ...
    const raw = await db.getEnergyData(id, new Date(from + "T00:00:00"), new Date(to + "T23:59:59"));
    if (!raw || raw.length === 0) return { kwh: 0, ils: 0, peak_kwh: 0, off_kwh: 0 };

    // Compute deltas
    const sorted = raw
      .map(r => ({ ae: Number(r.ActiveEnergy ?? r.activeenergy ?? 0), ts: new Date(r.timestamp ?? r.ts) }))
      .sort((a, b) => a.ts - b.ts);
...
// ── Build report data ─────────────────────────────────────
async function buildReportData(dateStr) {
  const yStr  = yesterdayStr();
  const lwStr = lastWeekStr();

  const [mainData, pb1Main, pb1Ac, yTotal, lwTotal] = await Promise.all([
    Promise.all(MAIN_BREAKERS.map(b => getBreaker(b.id, dateStr, dateStr))),
    getBreaker(28, dateStr, dateStr),
    getBreaker(29, dateStr, dateStr),
    Promise.all(MAIN_BREAKERS.map(b => getBreaker(b.id, yStr, yStr))),
    Promise.all(MAIN_BREAKERS.map(b => getBreaker(b.id, lwStr, lwStr))),
  ]);

  const panels = MAIN_BREAKERS.map((b, i) => ({ ...b, ...mainData[i] }));
  ...
  const yTotalKwh  = yTotal.reduce((s, d) => s + d.kwh, 0);
  const lwTotalKwh = lwTotal.reduce((s, d) => s + d.kwh, 0);
  ...
  const pb1Other = Math.max(0, pb1Main.kwh - pb1Ac.kwh);
  const pbDirect = Math.max(0, panels.find(p => p.id === 27)?.kwh - pb1Main.kwh || 0);

  return { panels, totalKwh, totalIls, totalPeak, totalOff, vsYesterday, vsLastWeek,
    yTotalKwh, lwTotalKwh, pb1Main, pb1Ac, pb1Other, pbDirect, dateStr };
}
...
function buildHtml(d) {
  ...
  const maxKwh = Math.max(...d.panels.map(p => p.kwh), 1);
...
.cost-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:10px}
...
.cost-name{font-size:10px;color:#555;margin-bottom:7px}
...
.tag-b0{background:#ffe8e8;color:#CC0010;display:inline-block;padding:2px 7px;border-radius:3px;font-size:9px;font-weight:700}
.tag-roof{background:#fff4e0;color:#b37000;display:inline-block;padding:2px 7px;border-radius:3px;font-size:9px;font-weight:700}
.tag-pb{background:#e8eeff;color:#2255bb;display:inline-block;padding:2px 7px;border-radius:3px;font-size:9px;font-weight:700}
.tag-pv{background:#e8f5ec;color:#1a7f37;display:inline-block;padding:2px 7px;border-radius:3px;font-size:9px;font-weight:700}
...
<div class="wrap">
  ...
  <div class="ts-bar">
    <div class="ts-item">OPC UA · <span>30 breakers</span></div>
  ...
  <div class="kpi-row">
    <div class="kpi-cell kpi-c1"><div class="kpi-label">Total consumption</div><div class="kpi-val">${fmtNum(d.totalKwh)}<span class="kpi-unit">kWh</span></div><div class="kpi-sub">4 main panels</div></div>
  ...
  <div class="body">
    ...
    <div class="peak-alert">
      ...
      <div class="peak-text"><strong>Peak hours:</strong> ${fmtNum(d.totalPeak)} kWh during shoulder peak (17:00–22:00) · ${fmtIls(d.totalPeak * 0.4293)} ILS. Off-peak: <strong>${fmtNum(d.totalOff)} kWh · ${fmtIls(d.totalOff * 0.3945)} ILS</strong></div>
    </div>

    <div class="sec"><div class="sec-label">Cost per panel</div><div class="sec-line"></div></div>
    <div class="cost-grid">
      ${d.panels.map(p => {
        const tagClass = `tag-${p.group.toLowerCase()}`;
        return `<div class="cost-card">
          <div class="cost-top" style="background:${p.color}11">
            ...
            <div class="cost-name">${p.name.replace(" — Main","").replace(" Panels","")}</div>
          ...
          <div class="cost-bot"><span class="cost-kwh">${fmtNum(p.kwh)} kWh</span><span class="cost-pct">${d.totalKwh > 0 ? Math.round(p.kwh/d.totalKwh*100) : 0}%</span></div>
        </div>`;
      }).join("")}
    </div>
    <div class="total-row">
      <div>
        <div class="total-label">Total site cost</div>
    ...
    <table class="panel-tbl">
      ...
      <tbody>
        ${d.panels.map((p, i) => {
          const yP = /* yesterday panel */ 0; // simplified
          const pct = d.totalKwh > 0 ? Math.round(p.kwh/d.totalKwh*100) : 0;
          return `<tr>
            <td><span class="tag-${p.group.toLowerCase()}">${p.group}</span> ${p.name.split("—")[0].trim()}</td>
    ...
    <div class="pb-tree">
      ...
      <div style="padding-left:14px;border-left:1px solid #1a1d22;margin-left:6px">
        <div class="pb-l2">
        ...
        <div style="padding-left:13px;border-left:1px dashed #1e2025;margin-left:3px">
          <div class="pb-l3">
    ...
    <div class="cmp-grid">
      <div class="cmp-box cmp-y">
        <div class="cmp-label">vs Yesterday (same hours)</div>
      ...
      <div class="cmp-box cmp-w">
        <div class="cmp-label">vs Last week same day</div>
